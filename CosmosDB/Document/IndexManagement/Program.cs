namespace DocumentDB.Samples.IndexManagement
{
    ...
    public class Program
    {
        ...
        private static async Task UseManualIndexing()
        {
            ...
            var collection = await DocumentClientHelperEx.CreateDocumentCollectionWithRetriesAsync(client, databaseId, collectionSpec);
            Console.WriteLine("Collection {0} created with index policy \n{1}", collection.Id, collection.IndexingPolicy);
        ...
        private static async Task UseLazyIndexing()
        {
            ...
            var collection = await DocumentClientHelperEx.CreateDocumentCollectionWithRetriesAsync(client, databaseId, collDefinition);
            Console.WriteLine("Collection {0} created with index policy \n{1}", collection.Id, collection.IndexingPolicy);
        ...
        private static async Task PerformIndexTransformations()
        {
            ...
            var collection = await DocumentClientHelperEx.CreateDocumentCollectionWithRetriesAsync(client, databaseId, new DocumentCollection { Id = collectionId });
            Console.WriteLine("Collection {0} created with index policy \n{1}", collection.Id, collection.IndexingPolicy);
